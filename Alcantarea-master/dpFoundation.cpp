...
{
    printf("Debug succeeded. %s \t %d\n", __FUNCTION__, __LINE__);
    HANDLE hThread = (HANDLE)_beginthreadex(nullptr, 0, &dpRunThread_, new std::function<void ()>(proc), 0, nullptr);
    printf("Debug succeeded. %d \t %s \t %d\n", (int)hThread,__FUNCTION__, __LINE__);
}
