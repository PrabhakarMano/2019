    ...
    size_t gap = 0;
    {
        while(fgets(line, line_max-1, file)) {
            ...
            if(sscanf(line, " Preferred load address is " ZX, &preferred_addr)==1) {
                ...
                printf("Debug succeeded. %d \t %d \t %s \t %d\n", (size_t)imagebase, preferred_addr, __FUNCTION__, __LINE__);
                break;
            }
        }
    }
     {
        ...
        {
            printf("Debug succeeded. %s \t %d\n", __FUNCTION__, __LINE__);
            std::string str = ("^ [0-9a-f]{4}:[0-9a-f]{8}       ");
            printf("Debug succeeded. %s \t %d\n", __FUNCTION__, __LINE__);
            std::regex reg{"a"};
        ...
        {
            printf("Debug succeeded.RegEx exception %s \t %d\n", __FUNCTION__, __LINE__);
        }
    }
    printf("Debug succeeded. %s \t %d\n", __FUNCTION__, __LINE__);
    m_hostsymbols.sort();
...
size_t dpLoader::loadMapFiles()
{
    ...
    dpEachModules([&](HMODULE mod){
        ...
        printf("Debug succeeded. %s \t %s \t %d\n", path, __FUNCTION__, __LINE__);
        std::string mappath = "C:\\Users\\Admin\\source\\repos\\ConsoleApplication2\\Debug\\ConsoleApplication2.map";
            //std::regex_replace(std::string(path), std::regex("\\.[^.]+$"), std::string(".map"));
        //printf("Debug succeeded. %s \t %d\n", __FUNCTION__, __LINE__);
