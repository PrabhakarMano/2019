...
HRESULT InitDevice()
{
...

    {
        ...
        hr = CompileShaderFromFile( L"Test_Particles\\Test_Particles.fx", "VS", "vs_4_0", &pVSBlob );
        if( FAILED( hr ) )
        {
            MessageBox( NULL,
...

    {
        ...
        hr = CompileShaderFromFile( L"Test_Particles\\Test_Particles.fx", "PS", "ps_4_0", &pPSBlob );
        if( FAILED( hr ) )
        {
            MessageBox( NULL,
                L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK );
...
void Render()
{
    ...
    g_pImmediateContext->DrawIndexedInstanced( 36, (UINT)g_num_particles, 0, 0, 0 );
...
#endif
#define dpObjDir "_tmp/Test_Particles_" dpPlatform dpConfiguration 
